    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const msg = (data.errors && data.errors.map(x => x.message).join(' ')) || 'Something went wrong. Please email Ilham@tutorselevate.co.uk instead.';
          status.textContent = msg;
          status.classList.add('is-error');
          status.hidden = false;
        }
      } catch (err) {
        status.textContent = 'Network error. Please email Ilham@tutorselevate.co.uk instead.';
        status.classList.add('is-error');
        status.hidden = false;
      } finally {
